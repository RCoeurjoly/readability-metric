...
#+STARTUP: oddeven
#+begin_quote
This is the legacy literate notebook for the original implementation. The maintained Python entry point is now =readability_metric.py= and the reproducible environment is in =flake.nix=.
#+end_quote
* Summary
