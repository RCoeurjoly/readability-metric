...
#+begin_src python :noweb yes :session python :exports code
language=cleantext.language.code
#+end_src
...
#+NAME: tokenization
#+BEGIN_SRC python :noweb yes :session python :exports code
if (language == 'zh' or language == 'zh-Hant'):
    zh_characters = ''.join(c for c in cleantext if u'\u4e00' <= c <= u'\u9fff')
tokens = cleantext.words
...
if word_count > 10000:
    ...
    popt, pcov = fit_values(log_log_func, sweep_values)
    intercept = popt[0]
    slope = popt[1]
...
if (language == 'zh' or language == 'zh-Hant'):
    for j in xrange(0, len(zh_characters) - start, (len(zh_characters) - start)/samples):
        sweep_values.append([len(zh_characters[0:start + j]), len(set(zh_characters[0:start + j]))])
    zhpopt, zhpcov = fit_values(log_func, sweep_values)
...
try:
    ...
    unique_words DECIMAL(10,3),
    zhslope DECIMAL(5,3),
    zhintercept DECIMAL(5,3),
    zhstd_error_slope DECIMAL(5,3),
    zhstd_error_intercept DECIMAL(5,3),
    character_count DECIMAL(10,3),
    unique_characters DECIMAL(10,3),
