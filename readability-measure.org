...
def extract_text(self, book):
    ...
    for item in book.get_items():
        if item.get_type() == ebooklib.ITEM_DOCUMENT:
            ...
            <<html-filtering>>
    cleantext = filter_non_printable(html_filtered)
    self.text = cleantext
#+END_SRC
...
def cleaning_non_printable(text):
    '''
...
def __init__(self, book):
    try:
        self.epub_type = book.get_metadata('DC', 'type')[0][0].encode('utf-8')
    except IndexError:
        self.epub_type = ''
    try:
        self.subject = book.get_metadata('DC', 'subject')[0][0].encode('utf-8')
    except IndexError:
        self.subject = ''
    try:
        self.source = book.get_metadata('DC', 'source')[0][0].encode('utf-8')
    except IndexError:
        self.source = ''
    try:
        self.rights = book.get_metadata('DC', 'rights')[0][0].encode('utf-8')
    except IndexError:
        self.rights = ''
    try:
        self.relation = book.get_metadata('DC', 'relation')[0][0].encode('utf-8')
    except IndexError:
        self.relation = ''
    try:
        self.publisher = book.get_metadata('DC', 'publisher')[0][0].encode('utf-8')
    except IndexError:
        self.publisher = ''
    #try:
    #    self.language = book.get_metadata('DC', 'language')[0][0].encode('utf-8')
    #except IndexError:
    #    self.language = 'empty'
    try:
        self.identifier = book.get_metadata('DC', 'identifier')[0][0].encode('utf-8')
    except IndexError:
        self.identifier = ''
    try:
        self.epubFormat = book.get_metadata('DC', 'format')[0][0].encode('utf-8')
    except IndexError:
        self.epubFormat = ''
    try:
        self.description = book.get_metadata('DC', 'description')[0][0].encode('utf-8')
    except IndexError:
        self.description = ''
    try:
        self.coverage = book.get_metadata('DC', 'coverage')[0][0].encode('utf-8')
    except IndexError:
        self.coverage = ''
    try:
        self.contributor = book.get_metadata('DC', 'contributor')[0][0].encode('utf-8')
    except IndexError:
        self.contributor = ''
    try:
        self.author = book.get_metadata('DC', 'creator')[0][0].encode('utf-8')
    except IndexError:
        self.author = ''
    try:
        self.title = book.get_metadata('DC', 'title')[0][0].encode('utf-8')
    except IndexError:
        self.title = ''
    try:
        self.date = book.get_metadata('DC', 'date')[0][0].encode('utf-8')
    except IndexError:
        self.date = ''
#+END_SRC
*** Tokenization
...
def tokenize(self, book):
    if self.language == 'zh' or self.language == 'zh_Hant':
        self.zh_characters = ''.join(character for character in self.cleantext if u'\u4e00' <= c <= u'\u9fff')
        self.character_count = len(zh_characters)
        self.unique_characters = len(set(zh_characters))
    else:
        self.zh_characters = str()
        self.character_count = int()
        self.unique_characters = int()
    self.tokens = Text(cleantext).words
    self.word_count = len(tokens)
    self.unique_words = len(set(tokens))
#+END_SRC
*** Log writing
#+NAME: log-writing
#+BEGIN_SRC python :noweb yes :session python :exports code
...
def curve_fit(function, sweep_values):
    popt, pcov = fit_values(function, sweep_values)
    intercept = popt[0]
    slope = popt[1]
    perr = np.sqrt(np.diag(pcov))
    std_error_intercept = perr[0]
    std_error_slope = perr[1]
    return intercept, slope, std_error_intercept, std_error_slope
#+END_SRC
